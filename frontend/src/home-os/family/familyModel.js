...
export function buildFamilyModel(entities = {}, now = new Date()) {
  ...
  return { people, openings, presence, cameras, appliances, batteries, consumables, schedule, todoLists, alerts: openings.filter((e) => e.open).length + batteries.filter((e) => e.value <= 20).length + consumables.filter((e) => e.value <= 15).length };
}
